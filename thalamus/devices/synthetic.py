...
#: kind -> factory(params, rng) -> f(t_seconds) -> value
...
def _random_walk(
    ...
    **_,
    ...
    """A drifting value, clamped to a range. The closest thing here to a mouse trace."""
    state = {"value": start}

    def next_value(_t: float) -> float:
        value = state["value"] + rng.gauss(0.0, step)
...
@generator("eeg")
def _eeg(*, amplitude: float = 20.0, alpha_freq: float = 10.0, rng: random.Random, **_):
    ...
    def value(t: float) -> float:
        alpha = amplitude * math.sin(2 * math.pi * alpha_freq * t + 0.3)
        noise = sum(
            (amplitude / (2.0 * freq)) * math.sin(2 * math.pi * freq * t + phase)
            for freq, phase in background
        )
...
class SyntheticDevice(RecordingDevice):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not signals:
            raise ValueError(f"[{device_id}] needs at least one signal")
    ...
    def _build(self, name: str, spec: Dict[str, Any]) -> Callable[[float], float]:
        ...
        return _GENERATORS[kind](rng=random.Random(self._rng.random()), **params)

    def describe(self) -> Dict[str, Any]:
        info = super().describe()
        info.setdefault("channels", list(self._signals))
        info.setdefault("synthetic", True)
        return info
